...
struct Model
{
	...
	bool randomPlacement(const int numAtoms, const int type, const double x, const double y, 
	...
	double getMetropolisProb(const double Einitial, const double Efinal, const double kT_local);
	double getRandomDecimal();
	unsigned int getRandomInteger();
	double cosijk(const Point &i, const Point &j, const Point &k,
		const double rij, const double rik);
	Point closestPointWithPBC(const Point &p1, const Point &p2);

	void performAnnealing(const int numSwaps, const int numVSwaps, string filename);
	void randomSwap();
	int relax();
	void relaxVolume();
	void swapBonds(const size_t aSubs, const size_t bSubs, 
		const size_t cSubs, const size_t dSubs);
	void initAtomsInShell();
	void initClosestNeighbors();

	void initShellNumbers(const size_t aSubs, const size_t bSubs, 
		const size_t cSubs, const size_t dSubs);
	bool isNearestNeighbor(const size_t id, const Atom &atom);
	void fourNearestNeighbors(const char *filename);
	bool isSecondNearestNeighbor(const size_t id, const size_t currentSubs);
	void keepInBox();
	...
	void writeInitialEnergy(string filename);
	void writeInitialForces(string filename);
